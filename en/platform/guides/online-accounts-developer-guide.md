...
The API to enumerate the accounts is provided by the `[AccountModel](../api-qml-current/Ubuntu.OnlineAccounts.2.AccountModel.md)` QML
...
access to the user's accounts, and this is done by invoking the [requestAccess()](../api-qml-current/Ubuntu.OnlineAccounts.2.AccountModel.md#requestAccess-method/) method:
...
authentication token is simply done by accessing an `[Account](../api-qml-current/Ubuntu.OnlineAccounts.2.Account.md)` element and
calling its `[authenticate())(../api-qml-current/Ubuntu.OnlineAccounts.2.Account.md#authenticate-method)` method:
...
emit the [authenticationReply()](../api-qml-current/Ubuntu.OnlineAccounts.2.Account.md#authenticationReply-signal) signal which will
...
type `[Account](../api-qml-current/Ubuntu.OnlineAccounts.Account.md)` which represents the account being created or edited. In order
