...
**Note:** The .import syntax doesn't work for scripts used in the [WorkerScript](../QtQml.WorkerScript.md)
...
Notice that calling [Qt.include()](../QtQml.Qt.md#include-method) copies all functions from `factorial.js` into the `MyScript` namespace, which means the QML component can also access `factorial()` directly as `MyScript.factorial()`.
